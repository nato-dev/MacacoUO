        ...
        private class InternalTarget : Target
        {
            ...
            private class InternalTimer : Timer
            {
                ...
                protected override void OnTick()
                {
                    ...
                    {
                        m_Tamer.RevealingAction(false);

                        m_Tamer.OverheadMessage("* Domando *");

                        if (!alreadyOwned) // Passively check animal lore for gain
                        {
                            m_Tamer.CheckTargetSkillMinMax(SkillName.AnimalLore, m_Creature, 0.0, 120.0);
                        }
                    ...
                    else
                    {
                        ...
                        var mult = 1d;
                        if(m_Tamer.Skills[SkillName.AnimalTaming].Value - 10 < m_Creature.MinTameSkill)
                        {
                            mult += 0.5;
                        }
                        if (m_Creature.MinTameSkill > 90)
                        {
                            mult += 0.5;
                        }
                        if (m_Creature.MinTameSkill > 100)
                        {
                            mult += 0.5;
                        }
                            ...
                            m_Tamer.CheckTargetSkillMinMax(SkillName.AnimalTaming, m_Creature, minSkill - 35.0, minSkill + 35.0, mult))
                        {
                            ...
                            if (alreadyOwned)
                            {
                                m_Tamer.SendLocalizedMessage("Este animal ja foi domado antes, nao foi muito desafiador"); // That wasn't even challenging.
                            }
                            else
                            {
