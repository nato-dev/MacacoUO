...
namespace Server
{
    public class LootPack
    {
        ...
        public static readonly LootPackItem[] LowNecroScrollItems = new[]
        {
            new LootPackItem(typeof(EvilOmenScroll), 1)
        };


        public static readonly LootPackItem[] MedNecroScrollsItems = new[]
        {
            new LootPackItem(typeof(CorpseSkinScroll), 1)
        };

        public static readonly LootPackItem[] HighNecroScrollItems = new[]
        {
            new LootPackItem(typeof(WraithFormScroll), 1)
        };


        public static readonly LootPackItem[] TintaBranca = new[]
    {
            new LootPackItem(typeof(TintaBranca), 1)
        };

        public static readonly LootPackItem[] TintaPreta = new[]
   {
            new LootPackItem(typeof(TintaPreta), 1)
        };
        ...
        {
            new LootPackItem(typeof(BaseJewel), 3), new LootPackItem(typeof(BaseArmor), 10),
            new LootPackItem(typeof(BaseWeapon), 8), new LootPackItem(typeof(BaseRanged), 3),
            new LootPackItem(typeof(BaseShield), 3),
            new LootPackItem(typeof(TintaMagica), 1)
        };
        #endregion
        ...
        public static readonly LootPack AosPoor = OldPoor;

        public static readonly LootPack AosMeager = OldMeager;

        public static readonly LootPack AosAverage = OldAverage;


        public static readonly LootPack AosRich = OldRich;


        public static readonly LootPack AosFilthyRich = OldFilthyRich;


        public static readonly LootPack AosUltraRich = OldUltraRich;


        public static readonly LootPack AosSuperBoss = OldSuperBoss;
        ...
        public static readonly LootPack OldPoor =
            new LootPack(new[] {
                new LootPackEntry(true, Gold, 100.00, "4d10+6"),
                new LootPackEntry(false, Instruments, 0.02, 1),
                new LootPackEntry(false, GemItems, 0.02, 1, 1, 1, 100)
            });
        ...
        public static readonly LootPack OldMeager =
            new LootPack(
                new[]
                {
                    ...
                    new LootPackEntry(false, GemItems, 3, 1, 1, 1, 100),
                });
        ...
        public static readonly LootPack OldAverage =
            new LootPack(
                new[]
                {
                    ...
                    new LootPackEntry(false, GemItems, 4, 1, 1, 1, 100),
                });

        public static readonly LootPack OldMagicItem =
            new LootPack(
                new[]
                {
        ...
        public static readonly LootPack MedNecroScrolls = new LootPack(new[] { new LootPackEntry(false, MedNecroScrollsItems, 100.00, 1) });

        public static readonly LootPack HighNecroScrolls =
            new LootPack(new[] { new LootPackEntry(false, HighNecroScrollItems, 100.00, 1) });
    ...
    public class LootPackEntry
    {
        ...
        public Item Construct(Mobile from, bool spawning)
        {
            ...
            foreach (LootPackItem item in Items)
            {
                if (rnd < item.Chance)
                {
                    return Mutate(from, 0, item.Construct(IsInTokuno(from), IsMondain(from), IsStygian(from)));
                    //return Mutate(from, item.Construct(IsInTokuno(from), IsMondain(from)));
                }
        ...
        public static void ItemMagico(Item item)
        {

        }

        public Item Mutate(Mobile from, int luckChance, Item item, bool randomSlayer = true)
        {
            if (item != null)
            {
    ...
    public class LootPackItem
    {
        ...
        public static Item RandomNecroScroll(int minCircle, int maxCircle)
        {
            ...
            int minIndex, maxIndex, rnd;
            SpellbookType spellBookType = SpellbookType.Necromancer;
        ...
        public Item Construct(bool inTokuno, bool isMondain, bool isStygian)
        {
            try
            {
                ...
                {
                    item = RandomScroll(6, 8);
                }

                // NECRO
                else if (m_Type == typeof(EvilOmenScroll)) // low scroll
                {
                    item = RandomNecroScroll(1, 3);
                }
                else if (m_Type == typeof(CorpseSkinScroll)) // med scroll
                {
                    item = RandomNecroScroll(2, 6);
                }
                else if (m_Type == typeof(WraithFormScroll)) // high scroll
                {
                    item = RandomNecroScroll(6, 8);
                }
