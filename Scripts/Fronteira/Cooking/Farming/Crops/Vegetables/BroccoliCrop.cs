...
{
	public class BroccoliSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new BroccoliSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class BroccoliSeedling : HerdingBaseCrop
	{
		...
		public static void init( BroccoliSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(BroccoliCrop), plant.Sower );
	...
	public class BroccoliCrop : HerdingBaseCrop
	{
		...
		public static void init ( BroccoliCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xC62 );
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Broccoli crop = new Broccoli( pick );
		...
		{
			private BroccoliCrop i_plant;
			public CropTimer( BroccoliCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Broccoli fruit = new Broccoli( Utility.Random( m_yield +2 ) );
