...
{
	public class SoySeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new SoySeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class SoySeedling : HerdingBaseCrop
	{
		...
		public static void init( SoySeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SoyCrop), plant.Sower );
	...
	public class SoyCrop : HerdingBaseCrop
	{
		...
		public static void init ( SoyCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xCB0 );
			plant.FullGraphic = ( 0xC7E );
		...
		{
			private SoyCrop i_plant;
			public CropTimer( SoyCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Soybean fruit = new Soybean( Utility.Random( m_yield +2 ) );
