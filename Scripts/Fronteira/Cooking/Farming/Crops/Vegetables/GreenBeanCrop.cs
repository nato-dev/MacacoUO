...
{
	public class GreenBeanSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new GreenBeanSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class GreenBeanSeedling : HerdingBaseCrop
	{
		...
		public static void init( GreenBeanSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(GreenBeanCrop), plant.Sower );
	...
	public class GreenBeanCrop : HerdingBaseCrop
	{
		...
		public static void init ( GreenBeanCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xD04 );
			plant.FullGraphic = ( 0xD04 );
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						GreenBean crop = new GreenBean( pick );
		...
		{
			private GreenBeanCrop i_plant;
			public CropTimer( GreenBeanCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						GreenBean fruit = new GreenBean( Utility.Random( m_yield +2 ) );
