...
{
	public class EggplantSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new EggplantSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class EggplantSeedling : HerdingBaseCrop
	{
		...
		public static void init( EggplantSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(EggplantCrop), plant.Sower );
	...
	public class EggplantCrop : HerdingBaseCrop
	{
		...
		public static void init ( EggplantCrop plant, bool full )
		{
			plant.PickGraphic = ( Utility.RandomList( 0xC9B, 0xC9D ) );
			plant.FullGraphic = ( Utility.RandomList( 0xC9B, 0xC9D ) );
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Eggplant crop = new Eggplant( pick );
		...
		{
			private EggplantCrop i_plant;
			public CropTimer( EggplantCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Eggplant fruit = new Eggplant( Utility.Random( m_yield +2 ) );
