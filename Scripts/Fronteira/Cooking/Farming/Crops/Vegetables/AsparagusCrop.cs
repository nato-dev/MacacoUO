...
{
	public class AsparagusSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new AsparagusSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class AsparagusSeedling : HerdingBaseCrop
	{
		...
		public static void init( AsparagusSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(AsparagusCrop), plant.Sower );
	...
	public class AsparagusCrop : HerdingBaseCrop
	{
		...
		public static void init ( AsparagusCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Asparagus crop = new Asparagus( pick );
		...
		{
			private AsparagusCrop i_plant;
			public CropTimer( AsparagusCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Asparagus fruit = new Asparagus( Utility.Random( m_yield +2 ) );
