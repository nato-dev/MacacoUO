...
{
	public class RadishSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new RadishSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class RadishSeedling : HerdingBaseCrop
	{
		...
		public static void init( RadishSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(RadishCrop), plant.Sower );
	...
	public class RadishCrop : HerdingBaseCrop
	{
		...
		public static void init ( RadishCrop plant, bool full )
		{
		...
		{
			private RadishCrop i_plant;
			public CropTimer( RadishCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Radish fruit = new Radish( Utility.Random( m_yield +2 ) );
