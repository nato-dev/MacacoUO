...
namespace Server.Items.Crops
{
	...
	public class TurnipSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new TurnipSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class TurnipSeedling : HerdingBaseCrop
	{
		...
		public static void init( TurnipSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(TurnipCrop), plant.Sower );
	...
	public class TurnipCrop : HerdingBaseCrop
	{
		...
		public static void init ( TurnipCrop plant, bool full )
		{
		...
		{
			private TurnipCrop i_plant;
			public CropTimer( TurnipCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Turnip fruit = new Turnip( Utility.Random( m_yield +2 ) );
