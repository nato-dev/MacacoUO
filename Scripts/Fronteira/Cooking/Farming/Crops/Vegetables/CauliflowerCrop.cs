...
{
	public class CauliflowerSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new CauliflowerSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class CauliflowerSeedling : HerdingBaseCrop
	{
		...
		public static void init( CauliflowerSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(CauliflowerCrop), plant.Sower );
	...
	public class CauliflowerCrop : HerdingBaseCrop
	{
		...
		public static void init ( CauliflowerCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xD06 );
			plant.FullGraphic = ( 0xD06 );
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Cauliflower crop = new Cauliflower( pick );
		...
		{
			private CauliflowerCrop i_plant;
			public CropTimer( CauliflowerCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Cauliflower fruit = new Cauliflower( Utility.Random( m_yield +2 ) );
