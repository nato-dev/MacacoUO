...
{
	public class OnionSeed : HerdingBaseCrop
	{
	...
	public class OnionSeedling : HerdingBaseCrop
	{
	...
	public class OnionCrop : HerdingBaseCrop
	{
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( OnionCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Onion fruit = new Onion( Utility.Random( m_yield +2 ) );
