...
{
	public class PeasSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new PeasSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class PeasSeedling : HerdingBaseCrop
	{
		...
		public static void init( PeasSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(PeasCrop), plant.Sower );
	...
	public class PeasCrop : HerdingBaseCrop
	{
		...
		public static void init ( PeasCrop plant, bool full )
		{
		...
		{
			private PeasCrop i_plant;
			public CropTimer( PeasCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Peas fruit = new Peas( Utility.Random( m_yield +2 ) );
