...
{
	public class SweetPotatoSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new SweetPotatoSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class SweetPotatoSeedling : HerdingBaseCrop
	{
		...
		public static void init( SweetPotatoSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SweetPotatoCrop), plant.Sower );
	...
	public class SweetPotatoCrop : HerdingBaseCrop
	{
		...
		public static void init ( SweetPotatoCrop plant, bool full )
		{
		...
		{
			private SweetPotatoCrop i_plant;
			public CropTimer( SweetPotatoCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						SweetPotato fruit = new SweetPotato( Utility.Random( m_yield +2 ) );
