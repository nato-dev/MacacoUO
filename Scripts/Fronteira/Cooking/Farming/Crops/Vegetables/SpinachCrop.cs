...
{
	public class SpinachSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new SpinachSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class SpinachSeedling : HerdingBaseCrop
	{
		...
		public static void init( SpinachSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SpinachCrop), plant.Sower );
	...
	public class SpinachCrop : HerdingBaseCrop
	{
		...
		public static void init ( SpinachCrop plant, bool full )
		{
			plant.PickGraphic = ( Utility.RandomList( 0xD09, 0xD0A ) );
			plant.FullGraphic = ( 0xD0B );
		...
		{
			private SpinachCrop i_plant;
			public CropTimer( SpinachCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Spinach fruit = new Spinach( Utility.Random( m_yield +2 ) );
