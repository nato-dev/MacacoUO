...
{
	public class SnowPeasSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new SnowPeasSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class SnowPeasSeedling : HerdingBaseCrop
	{
		...
		public static void init( SnowPeasSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SnowPeasCrop), plant.Sower );
	...
	public class SnowPeasCrop : HerdingBaseCrop
	{
		...
		public static void init ( SnowPeasCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xCF2 );
			plant.FullGraphic = ( 0xCEF );
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						SnowPeas crop = new SnowPeas( pick );
		...
		{
			private SnowPeasCrop i_plant;
			public CropTimer( SnowPeasCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						SnowPeas fruit = new SnowPeas( Utility.Random( m_yield +2 ) );
