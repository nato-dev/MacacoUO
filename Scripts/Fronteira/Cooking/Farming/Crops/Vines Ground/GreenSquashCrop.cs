...
{
	public class GreenSquashSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new GreenSquashSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class GreenSquashSeedling : HerdingBaseCrop
	{
		...
		public static void init( GreenSquashSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(GreenSquashCrop), plant.Sower );
	...
	public class GreenSquashCrop : HerdingBaseCrop
	{
		...
		[Constructable]
		public GreenSquashCrop( Mobile sower ) : base( 15374 )
		{
			Movable = false;
		...
		public static void init ( GreenSquashCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						GreenSquash crop = new GreenSquash( pick );
		...
		{
			private GreenSquashCrop i_plant;
			public CropTimer( GreenSquashCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						GreenSquash fruit = new GreenSquash( Utility.Random( m_yield +2 ) );
