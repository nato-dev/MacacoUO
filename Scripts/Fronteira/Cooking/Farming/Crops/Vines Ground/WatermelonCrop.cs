...
namespace Server.Items.Crops
{
    ...
    public class WatermelonSeed : HerdingBaseCrop
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new WatermelonSeedling(from);
            item.Location = m_pnt;
            item.Map = m_map;
        }
    ...
    public class WatermelonSeedling : HerdingBaseCrop
    {
        ...
        {
            plant.thisTimer = new CropHelper.GrowTimer(plant, typeof(WatermelonCrop), plant.Sower);
    ...
    public class WatermelonCrop : HerdingBaseCrop
    {
        ...
        [Constructable]
        public WatermelonCrop(Mobile sower) : base(15374)
        {
            Movable = false;
        ...
        private class CropTimer : Timer
        {
            ...
            public CropTimer(WatermelonCrop plant) : base(TimeSpan.FromSeconds(450), TimeSpan.FromSeconds(15))
            {
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if (from == m_sower)
                {
                    ...
                    {
                        Watermelon fruit = new Watermelon(Utility.Random(m_yield + 2));
