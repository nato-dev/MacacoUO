...
{
	public class HoneydewMelonSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new HoneydewMelonSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class HoneydewMelonSeedling : HerdingBaseCrop
	{
		...
		public static void init( HoneydewMelonSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(HoneydewMelonCrop), plant.Sower );
	...
	public class HoneydewMelonCrop : HerdingBaseCrop
	{
		...
		[Constructable]
		public HoneydewMelonCrop( Mobile sower ) : base( 15374 )
		{
			Movable = false;
		...
		public static void init ( HoneydewMelonCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						HoneydewMelon crop = new HoneydewMelon( pick );
		...
		{
			private HoneydewMelonCrop i_plant;
			public CropTimer( HoneydewMelonCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						HoneydewMelon fruit = new HoneydewMelon( Utility.Random( m_yield +2 ) );
