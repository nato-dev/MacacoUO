...
{
	public class RedMushroomSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new RedMushroomSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class RedMushroomSeedling : HerdingBaseCrop
	{
		...
		public static void init( RedMushroomSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(RedMushroomCrop), plant.Sower );
	...
	public class RedMushroomCrop : HerdingBaseCrop
	{
		...
		public static void init ( RedMushroomCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xD16 );
			plant.FullGraphic = ( 0xD16 );
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						RedMushroom crop = new RedMushroom( pick );
		...
		{
			private RedMushroomCrop i_plant;
			public CropTimer( RedMushroomCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						RedMushroom fruit = new RedMushroom( Utility.Random( m_yield +2 ) );
