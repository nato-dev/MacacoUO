...
{
	public class TanMushroomSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new TanMushroomSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class TanMushroomSeedling : HerdingBaseCrop
	{
		...
		public static void init( TanMushroomSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(TanMushroomCrop), plant.Sower );
	...
	public class TanMushroomCrop : HerdingBaseCrop
	{
		...
		public static void init ( TanMushroomCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xD13 );
			plant.FullGraphic = ( 0xD13 );
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						TanMushroom crop = new TanMushroom( pick );
		...
		{
			private TanMushroomCrop i_plant;
			public CropTimer( TanMushroomCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						TanMushroom fruit = new TanMushroom( Utility.Random( m_yield +2 ) );
