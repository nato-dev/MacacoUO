...
{
	public class NightshadeSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new NightshadeSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class NightshadeSeedling : HerdingBaseCrop
	{
		...
		public static void init( NightshadeSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(NightshadeCrop), plant.Sower );
	...
	public class NightshadeCrop : HerdingBaseCrop
	{
		...
		{
			private NightshadeCrop i_plant;
			public CropTimer( NightshadeCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Nightshade fruit = new Nightshade( Utility.Random( m_yield +2 ) );
