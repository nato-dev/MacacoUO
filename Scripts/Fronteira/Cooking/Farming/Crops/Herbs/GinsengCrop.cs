...
{
	public class GinsengSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new GinsengSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class GinsengSeedling : HerdingBaseCrop
	{
		...
		public static void init( GinsengSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(GinsengCrop), plant.Sower );
	...
	public class GinsengCrop : HerdingBaseCrop
	{
		...
		public static void init ( GinsengCrop plant, bool full )
		{
			plant.PickGraphic = ( 0xC7E );
			plant.FullGraphic = ( 0x18EA );
		...
		{
			private GinsengCrop i_plant;
			public CropTimer( GinsengCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Ginseng fruit = new Ginseng( Utility.Random( m_yield +2 ) );
