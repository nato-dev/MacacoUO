...
{
	public class MandrakeSeed : HerdingBaseCrop
	{
	...
	public class MandrakeSeedling : HerdingBaseCrop
	{
	...
	public class MandrakeCrop : HerdingBaseCrop
	{
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( MandrakeCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						MandrakeRoot fruit = new MandrakeRoot( Utility.Random( m_yield +2 ) );
