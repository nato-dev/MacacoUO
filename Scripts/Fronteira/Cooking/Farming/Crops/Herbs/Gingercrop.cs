...
{
	public class TanGingerSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new TanGingerSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class TanGingerSeedling : HerdingBaseCrop
	{
		...
		public static void init( TanGingerSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(TanGingerCrop), plant.Sower );
	...
	public class TanGingerCrop : HerdingBaseCrop
	{
		...
		public static void init ( TanGingerCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						TanGinger crop = new TanGinger( pick );
		...
		{
			private TanGingerCrop i_plant;
			public CropTimer( TanGingerCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						TanGinger fruit = new TanGinger( Utility.Random( m_yield +2 ) );
