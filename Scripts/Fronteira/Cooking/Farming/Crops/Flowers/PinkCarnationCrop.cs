...
{
	public class PinkCarnationSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new PinkCarnationSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class PinkCarnationSeedling : HerdingBaseCrop
	{
		...
		public static void init( PinkCarnationSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(PinkCarnationCrop), plant.Sower );
	...
	public class PinkCarnationCrop : HerdingBaseCrop
	{
		...
		public static void init ( PinkCarnationCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						PinkCarnation crop = new PinkCarnation();
						from.AddToBackpack( crop );
						if (pick == 2)
						{
							PinkCarnation crop2 = new PinkCarnation();
		...
		{
			private PinkCarnationCrop i_plant;
			public CropTimer( PinkCarnationCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						PinkCarnation fruit = new PinkCarnation( Utility.Random( m_yield +2 ) );
