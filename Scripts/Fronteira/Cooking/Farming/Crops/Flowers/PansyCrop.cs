...
{
	public class PansySeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new PansySeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class PansySeedling : HerdingBaseCrop
	{
		...
		public static void init( PansySeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(PansyCrop), plant.Sower );
	...
	public class PansyCrop : HerdingBaseCrop
	{
		...
		public static void init ( PansyCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Pansy crop = new Pansy();
						from.AddToBackpack( crop );
						if (pick == 2)
						{
							Pansy crop2 = new Pansy();
		...
		{
			private PansyCrop i_plant;
			public CropTimer( PansyCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Pansy fruit = new Pansy( Utility.Random( m_yield +2 ) );
