...
{
	public class SpiritRoseSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new SpiritRoseSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class SpiritRoseSeedling : HerdingBaseCrop
	{
		...
		public static void init( SpiritRoseSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SpiritRoseCrop), plant.Sower );
	...
	public class SpiritRoseCrop : HerdingBaseCrop
	{
		...
		public static void init ( SpiritRoseCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						SpiritRose crop = new SpiritRose();
						from.AddToBackpack( crop );
						if (pick == 2)
						{
							SpiritRose crop2 = new SpiritRose();
		...
		{
			private SpiritRoseCrop i_plant;
			public CropTimer( SpiritRoseCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						SpiritRose fruit = new SpiritRose( Utility.Random( m_yield +2 ) );
