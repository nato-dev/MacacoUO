...
{
	public class WhiteRoseSeed : HerdingBaseCrop
	{
	...
	public class WhiteRoseSeedling : HerdingBaseCrop
	{
	...
	public class WhiteRoseCrop : HerdingBaseCrop
	{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						WhiteRose2 crop = new WhiteRose2();
						from.AddToBackpack( crop );
						if (pick == 2)
						{
							WhiteRose2 crop2 = new WhiteRose2();
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						WhiteRose2 fruit = new WhiteRose2( Utility.Random( m_yield +2 ) );
