...
{
	public class RedRoseSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new RedRoseSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class RedRoseSeedling : HerdingBaseCrop
	{
		...
		public static void init( RedRoseSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(RedRoseCrop), plant.Sower );
	...
	public class RedRoseCrop : HerdingBaseCrop
	{
		...
		public static void init ( RedRoseCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						RedRose crop = new RedRose();
						from.AddToBackpack( crop );
						if (pick == 2)
						{
							RedRose crop2 = new RedRose();
		...
		{
			private RedRoseCrop i_plant;
			public CropTimer( RedRoseCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						RedRose fruit = new RedRose( Utility.Random( m_yield +2 ) );
