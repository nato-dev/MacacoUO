...
{
	public class BlackRoseSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new BlackRoseSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class BlackRoseSeedling : HerdingBaseCrop
	{
		...
		public static void init( BlackRoseSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(BlackRoseCrop), plant.Sower );
	...
	public class BlackRoseCrop : HerdingBaseCrop
	{
		...
		public static void init ( BlackRoseCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						BlackRose2 crop = new BlackRose2();
						from.AddToBackpack( crop );
						if (pick == 2)
						{
							BlackRose2 crop2 = new BlackRose2();
		...
		{
			private BlackRoseCrop i_plant;
			public CropTimer( BlackRoseCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						BlackRose2 fruit = new BlackRose2( Utility.Random( m_yield +2 ) );
