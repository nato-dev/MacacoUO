...
{
	public class SnapdragonSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new SnapdragonSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class SnapdragonSeedling : HerdingBaseCrop
	{
		...
		public static void init( SnapdragonSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SnapdragonCrop), plant.Sower );
	...
	public class SnapdragonCrop : HerdingBaseCrop
	{
		...
		public static void init ( SnapdragonCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Snapdragon crop = new Snapdragon();
						from.AddToBackpack( crop );
						if (pick == 2)
						{
							Snapdragon crop2 = new Snapdragon();
		...
		{
			private SnapdragonCrop i_plant;
			public CropTimer( SnapdragonCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Snapdragon fruit = new Snapdragon( Utility.Random( m_yield +2 ) );
