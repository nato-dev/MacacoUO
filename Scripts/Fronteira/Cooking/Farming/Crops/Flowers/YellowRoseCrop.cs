...
{
	public class YellowRoseSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new YellowRoseSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class YellowRoseSeedling : HerdingBaseCrop
	{
		...
		public static void init( YellowRoseSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(YellowRoseCrop), plant.Sower );
	...
	public class YellowRoseCrop : HerdingBaseCrop
	{
		...
		public static void init ( YellowRoseCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						YellowRose crop = new YellowRose();
						from.AddToBackpack( crop );
						if (pick == 2)
						{
							YellowRose crop2 = new YellowRose();
		...
		{
			private YellowRoseCrop i_plant;
			public CropTimer( YellowRoseCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						YellowRose fruit = new YellowRose( Utility.Random( m_yield +2 ) );
