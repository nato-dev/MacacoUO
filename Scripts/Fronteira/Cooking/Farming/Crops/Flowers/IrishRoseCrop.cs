...
{
	public class IrishRoseSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new IrishRoseSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class IrishRoseSeedling : HerdingBaseCrop
	{
		...
		public static void init( IrishRoseSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(IrishRoseCrop), plant.Sower );
	...
	public class IrishRoseCrop : HerdingBaseCrop
	{
		...
		public static void init ( IrishRoseCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						IrishRose crop = new IrishRose();
						from.AddToBackpack( crop );
						if (pick == 2)
						{
							IrishRose crop2 = new IrishRose();
		...
		{
			private IrishRoseCrop i_plant;
			public CropTimer( IrishRoseCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						IrishRose fruit = new IrishRose( Utility.Random( m_yield +2 ) );
