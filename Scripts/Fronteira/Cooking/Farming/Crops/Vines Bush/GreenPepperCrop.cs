...
{
	public class GreenPepperSeed : HerdingBaseCrop
	{
	...
	public class GreenPepperSeedling : HerdingBaseCrop
	{
		...
		[Constructable]
		public GreenPepperSeedling( Mobile sower ) : base( 0xC61 )
		{
			Movable = false;
	...
	public class GreenPepperCrop : HerdingBaseCrop
	{
		...
		[Constructable]
		public GreenPepperCrop() : this(null) { }

		[Constructable]
		public GreenPepperCrop( Mobile sower ) : base( 3213 )
		{
			Movable = false;
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( GreenPepperCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						GreenPepper fruit = new GreenPepper( Utility.Random( m_yield +2 ) );
