...
{
	public class ChiliPepperSeed : HerdingBaseCrop
	{
		public override bool CanGrowGarden{ get{ return true; } }

		[Constructable]
		public ChiliPepperSeed() : this( 1 ) { }

		[Constructable]
		public ChiliPepperSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true;
			Weight = .1;
			Hue = 0x5E2;
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new ChiliPepperSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}

		public ChiliPepperSeed( Serial serial ) : base( serial ) { }
	...
	public class ChiliPepperSeedling : HerdingBaseCrop
	{
		...
		[Constructable]
		public ChiliPepperSeedling( Mobile sower ) : base( 0xC61 )
		{
			Movable = false;
			Name = "Chili Pepper Seedling";
			m_sower = sower;
			init( this );
		}
		public static void init( ChiliPepperSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(ChiliPepperCrop), plant.Sower );
		...
		public ChiliPepperSeedling( Serial serial ) : base( serial ) { }
	...
	public class ChiliPepperCrop : HerdingBaseCrop
	{
		...
		[Constructable]
		public ChiliPepperCrop() : this(null) { }

		[Constructable]
		public ChiliPepperCrop( Mobile sower ) : base( 3213 )
		{
			Movable = false;
		...
		public static void init ( ChiliPepperCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						ChiliPepper crop = new ChiliPepper( pick );
		...
		{
			private ChiliPepperCrop i_plant;
			public CropTimer( ChiliPepperCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						ChiliPepper fruit = new ChiliPepper( Utility.Random( m_yield +2 ) );
		...
		public ChiliPepperCrop( Serial serial ) : base( serial ) { }
