...
{
	public class TomatoSeed : HerdingBaseCrop
	{
		public override bool CanGrowGarden{ get{ return true; } }

		[Constructable]
		public TomatoSeed() : this( 1 ) { }

		[Constructable]
		public TomatoSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true;
			Weight = .1;
			Hue = 0x5E2;
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new TomatoSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}

		public TomatoSeed( Serial serial ) : base( serial ) { }
	...
	public class TomatoSeedling : HerdingBaseCrop
	{
		...
		[Constructable]
		public TomatoSeedling( Mobile sower ) : base( 0xDEC )
		{
			Movable = false;
		...
		public static void init( TomatoSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(TomatoCrop), plant.Sower );
		...
		public TomatoSeedling( Serial serial ) : base( serial ) { }
	...
	public class TomatoCrop : HerdingBaseCrop
	{
		...
		[Constructable]
		public TomatoCrop() : this(null) { }

		[Constructable]
		public TomatoCrop( Mobile sower ) : base( 3566 )
		{
			Movable = false;
			Name = "Tomato Plant";
			//Hue = 2993;
		...
		public static void init ( TomatoCrop plant, bool full )
		{
			plant.PickGraphic = ( 3565 );
            plant.FullGraphic = ( 3566 );
		...
		{
			private TomatoCrop i_plant;
			public CropTimer( TomatoCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Tomato fruit = new Tomato( Utility.Random( m_yield +2 ) );
		...
		public TomatoCrop( Serial serial ) : base( serial ) { }
