...
{
	public class YellowPepperSeed : HerdingBaseCrop
	{
		public override bool CanGrowGarden{ get{ return true; } }

		[Constructable]
		public YellowPepperSeed() : this( 1 ) { }

		[Constructable]
		public YellowPepperSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true;
			Weight = .1;
			Hue = 0x5E2;
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new YellowPepperSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}

		public YellowPepperSeed( Serial serial ) : base( serial ) { }
	...
	public class YellowPepperSeedling : HerdingBaseCrop
	{
		...
		[Constructable]
		public YellowPepperSeedling( Mobile sower ) : base( 0xC61 )
		{
			Movable = false;
			Name = "Yellow Pepper Seedling";
			m_sower = sower;
			init( this );
		}
		public static void init( YellowPepperSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(YellowPepperCrop), plant.Sower );
		...
		public YellowPepperSeedling( Serial serial ) : base( serial ) { }
	...
	public class YellowPepperCrop : HerdingBaseCrop
	{
		...
		[Constructable]
		public YellowPepperCrop() : this(null) { }

		[Constructable]
		public YellowPepperCrop( Mobile sower ) : base( 3213 )
		{
			Movable = false;
			Name = "Yellow Pepper Plant";
			Hue = 2858;
			m_sower = sower;
		...
		public static void init ( YellowPepperCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						YellowPepper crop = new YellowPepper( pick );
		...
		{
			private YellowPepperCrop i_plant;
			public CropTimer( YellowPepperCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						YellowPepper fruit = new YellowPepper( Utility.Random( m_yield +2 ) );
		...
		public YellowPepperCrop( Serial serial ) : base( serial ) { }
