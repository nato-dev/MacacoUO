...
{
	public class OrangePepperSeed : HerdingBaseCrop
	{
		public override bool CanGrowGarden{ get{ return true; } }

		[Constructable]
		public OrangePepperSeed() : this( 1 ) { }

		[Constructable]
		public OrangePepperSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true;
			Weight = .1;
			Hue = 0x5E2;
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new OrangePepperSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}

		public OrangePepperSeed( Serial serial ) : base( serial ) { }
	...
	public class OrangePepperSeedling : HerdingBaseCrop
	{
		...
		[Constructable]
		public OrangePepperSeedling( Mobile sower ) : base( 0xC61 )
		{
			Movable = false;
			Name = "Orange Pepper Seedling";
			m_sower = sower;
			init( this );
		}
		public static void init( OrangePepperSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(OrangePepperCrop), plant.Sower );
		...
		public OrangePepperSeedling( Serial serial ) : base( serial ) { }
	...
	public class OrangePepperCrop : HerdingBaseCrop
	{
		...
		[Constructable]
		public OrangePepperCrop() : this(null) { }

		[Constructable]
		public OrangePepperCrop( Mobile sower ) : base( 3213 )
		{
			Movable = false;
			Name = "Orange Pepper Plant";
			Hue = 2940;
			m_sower = sower;
		...
		public static void init ( OrangePepperCrop plant, bool full )
		{
			plant.PickGraphic = ( 3213 );
			plant.FullGraphic = ( 3213 );
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						OrangePepper crop = new OrangePepper( pick );
		...
		{
			private OrangePepperCrop i_plant;
			public CropTimer( OrangePepperCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						OrangePepper fruit = new OrangePepper( Utility.Random( m_yield +2 ) );
		...
		public OrangePepperCrop( Serial serial ) : base( serial ) { }
