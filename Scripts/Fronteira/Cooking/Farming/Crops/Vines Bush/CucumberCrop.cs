...
{
	public class CucumberSeed : HerdingBaseCrop
	{
		public override bool CanGrowGarden{ get{ return true; } }

		[Constructable]
		public CucumberSeed() : this( 1 ) { }

		[Constructable]
		public CucumberSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true;
			Weight = .1;
			Hue = 0x5E2;
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new CucumberSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}

		public CucumberSeed( Serial serial ) : base( serial ) { }
	...
	public class CucumberSeedling : HerdingBaseCrop
	{
		...
		[Constructable]
		public CucumberSeedling( Mobile sower ) : base( 0xC61 )
		{
			Movable = false;
		...
		public static void init( CucumberSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(CucumberCrop), plant.Sower );
		...
		public CucumberSeedling( Serial serial ) : base( serial ) { }
	...
	public class CucumberCrop : HerdingBaseCrop
	{
		...
		[Constructable]
		public CucumberCrop() : this(null) { }

		[Constructable]
		public CucumberCrop( Mobile sower ) : base( 3213 )
		{
			Movable = false;
			Name = "Cucumber Plant";
			Hue = 2998;
		...
		public static void init ( CucumberCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						Cucumber crop = new Cucumber( pick );
		...
		{
			private CucumberCrop i_plant;
			public CropTimer( CucumberCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Cucumber fruit = new Cucumber( Utility.Random( m_yield +2 ) );
		...
		public CucumberCrop( Serial serial ) : base( serial ) { }
