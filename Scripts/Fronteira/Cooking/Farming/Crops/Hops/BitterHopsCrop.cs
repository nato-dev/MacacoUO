...
{
	public class BitterHopsSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new BitterHopsSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class BitterHopsSeedling : HerdingBaseCrop
	{
		...
		public static void init( BitterHopsSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(BitterHopsCrop), plant.Sower );
	...
	public class BitterHopsCrop : HerdingBaseCrop
	{
		...
		public static void init ( BitterHopsCrop plant, bool full )
		{
		...
		{
			private BitterHopsCrop i_plant;
			public CropTimer( BitterHopsCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						BitterHops fruit = new BitterHops( Utility.Random( m_yield +2 ) );
