...
{
	public class SnowHopsSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new SnowHopsSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class SnowHopsSeedling : HerdingBaseCrop
	{
		...
		public static void init( SnowHopsSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SnowHopsCrop), plant.Sower );
	...
	public class SnowHopsCrop : HerdingBaseCrop
	{
		...
		public static void init ( SnowHopsCrop plant, bool full )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
						...
						SnowHops crop = new SnowHops( pick );
		...
		{
			private SnowHopsCrop i_plant;
			public CropTimer( SnowHopsCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						SnowHops fruit = new SnowHops( Utility.Random( m_yield +2 ) );
