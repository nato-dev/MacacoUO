...
{
	public class SweetHopsSeed : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			this.Consume();
			Item item = new SweetHopsSeedling( from );
			item.Location = m_pnt;
			item.Map = m_map;
		}
	...
	public class SweetHopsSeedling : HerdingBaseCrop
	{
		...
		public static void init( SweetHopsSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SweetHopsCrop), plant.Sower );
	...
	public class SweetHopsCrop : HerdingBaseCrop
	{
		...
		public static void init ( SweetHopsCrop plant, bool full )
		{
		...
		{
			private SweetHopsCrop i_plant;
			public CropTimer( SweetHopsCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						SweetHops fruit = new SweetHops( Utility.Random( m_yield +2 ) );
