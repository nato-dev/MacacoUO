...
namespace Server.Mobiles
{
    ...
    public abstract class BaseAI
    {
        ...
        public virtual void EndPickTarget(Mobile from, IDamageable target, OrderType order)
        {
            ...
            {
                if (m_Mobile.ControlMaster.IsHarmfulCriminal(target))
                {
                    m_Mobile.ControlMaster.CriminalAction(false);
                    m_Mobile.CriminalAction(false);
                }
        ...
        private static readonly SkillName[] m_KeywordTable =
        {
            ...
            SkillName.ItemID, SkillName.ArmsLore, SkillName.Begging, SkillName.Blacksmith, SkillName.Bowcraft,
        ...
        public virtual void OnSpeech(SpeechEventArgs e)
        {
            if (e.Mobile.Alive && e.Mobile.InRange(m_Mobile.Location, 3) && m_Mobile.IsHumanInTown())
            {
                ...
                {
                    if (m_Mobile is MondainQuester && e.Mobile is PlayerMobile)
                    {
                        ((MondainQuester)m_Mobile).OnTalk((PlayerMobile)e.Mobile);
                    }
                    else
                    {
                        m_Mobile.Say("Eu nao sei nada de aventuras...");
                    }
                }
                else if (e.Speech.Contains("contratar") || e.Speech.Contains("contrato"))
                {
                    if (m_Mobile is BaseHire)
                    {
                        var hire = (BaseHire)m_Mobile;
                        if (!hire.IsHired)
                        {
                            hire.Payday(hire);
                            hire.SayHireCost();
        ...
        public virtual bool DoOrderFollow()
        {
            ...
            else if (m_Mobile.ControlTarget != null && !m_Mobile.ControlTarget.Deleted && m_Mobile.ControlTarget != m_Mobile)
            {
                ...
                else
                {
                    ...
                    {
                        if (bRun)
                            m_Mobile.CurrentSpeed = 0.05;
                        else
                            m_Mobile.CurrentSpeed = FollowSpeed;

                        Shard.Debug("Follow Speed " + m_Mobile.CurrentSpeed, master);
                    }
        ...
        {
            m_Mobile.DebugSay("Ordem de ataque !");
            if (m_Mobile.IsDeadPet)
            {
                return true;
            }
