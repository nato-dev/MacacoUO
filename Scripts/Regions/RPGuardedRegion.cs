...
{
	public class RPGuardedRegion : BaseRegion
	{
		...
		public RPGuardedRegion(string name, Map map, int priority, params Rectangle3D[] area)
			: base(name, map, priority, area)
		{
			m_GuardType = DefaultGuardType;
		}

		public RPGuardedRegion(string name, Map map, int priority, params Rectangle2D[] area)
			: base(name, map, priority, area)
		{
			m_GuardType = DefaultGuardType;
		}

		public RPGuardedRegion(XmlElement xml, Map map, Region parent)
			: base(xml, map, parent)
		{
		...
		{
            m.SendMessage(78, "Voce entrou em uma cidade RP");
			if (IsDisabled())
			{
				return;
			}

			if (!AllowReds && m.Murderer && !m.RP)
			{
