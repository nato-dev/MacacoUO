...
{
	public class DungeonGuardedRegion : BaseRegion
	{
        ...
        public override void OnDeath(Mobile m)
        {
            ...
            if(m is BaseCreature)
            {
                ...
                foreach(var item in m.Backpack.Items.ToArray())
                {
                    ...
                    if (item is Gold)
                    {
                        item.Amount = (int)(item.Amount * 0.75);
                        if (item.Amount <= 6)
                            item.Amount = 6;
                    } else
                    {
        ...
        public DungeonGuardedRegion(string name, Map map, int priority, params Rectangle3D[] area)
			: base(name, map, priority, area)
		{
			m_GuardType = DefaultGuardType;
		}

		public DungeonGuardedRegion(string name, Map map, int priority, params Rectangle2D[] area)
			: base(name, map, priority, area)
		{
			m_GuardType = DefaultGuardType;
		}

		public DungeonGuardedRegion(XmlElement xml, Map map, Region parent)
			: base(xml, map, parent)
		{
        ...
        public List<Mobile> GetInRange(Point3D focus, int r)
        {

            return Map.GetMobilesInRange(focus, r).Where(m => m is PlayerMobile).Select(m => m).ToList();
        }


        public List<Mobile> GetInRange(Mobile focus, int r)
        {
		...
		{
			if (m is BaseCreature || m is BaseGuard || m.GuardImmune || !m.Alive || m.IsStaff() || m.Blessed || (m is BaseCreature && ((BaseCreature)m).IsInvulnerable) ||
				IsDisabled())
			{
				return false;
			}
