...
namespace Server.Regions
{
    ...
    public class HolyRegion : BaseRegion
    {
        private static readonly object[] m_GuardParams = new object[1];
        ...
        public HolyRegion(string name, Map map, int priority, params Rectangle3D[] area)
            : base(name, map, priority, area)
        {
            m_GuardType = DefaultGuardType;
        }

        public HolyRegion(string name, Map map, int priority, params Rectangle2D[] area)
            : base(name, map, priority, area)
        {
            m_GuardType = DefaultGuardType;
        }

        public HolyRegion(XmlElement xml, Map map, Region parent)
            : base(xml, map, parent)
        {
            XmlElement el = xml["guards"];
