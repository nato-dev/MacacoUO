...
namespace Server.Misc
{
    public class VirtueArtifactsSystem
    {
        ...
        private static Type[] m_VirtueArtifacts = new Type[]
            {
                /*
                typeof( KatrinasCrook ), typeof( JaanasStaff ), typeof( DragonsEnd ),,
            ...
            if (chance > roll)
            {
                ...
                if (i != null)
                {
                    ...
                    if (!pm.PlaceInBackpack(i))
                    {
                        ...
                        {
                            pm.SendLocalizedMessage("Voce achou um item epico mas sua mochila e banco estao muito cheios"); // You find an artifact, but your backpack and bank are too full to hold it.
