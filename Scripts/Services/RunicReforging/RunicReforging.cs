...
namespace Server.Items
{
    ...
    public static class RunicReforging
    {
                ...
                case CraftResource.Pinho:
                    {
                        ...
                        if (index == 12 && tool.Resource == CraftResource.Cobre && (options & ReforgingOption.Structural) != 0 &&
                                                                                    (options & ReforgingOption.Fundamental) != 0)
                            return false;
        ...
        private static int GetResourceIndex(CraftResource resource)
        {
            ...
            {
                default:
                case CraftResource.Berilo: return 0;
                case CraftResource.Vibranium: return 1;
                case CraftResource.Bronze:
                case CraftResource.Dourado:
                case CraftResource.Niobio:
                case CraftResource.Lazurita:
                case CraftResource.Quartzo:
                case CraftResource.Cobre: return 2;
                case CraftResource.CouroHorned:
                case CraftResource.CouroBarbed:
                case CraftResource.CouroSpinned: return 3;
                case CraftResource.Carvalho: return 4;
                case CraftResource.Mogno:
                case CraftResource.Eucalipto:
                case CraftResource.Carmesim:
                case CraftResource.Gelo:
                case CraftResource.Pinho: return 5;
            }
        }
        ...
        public static bool GenerateRandomItem(Item item, int luck, int minBudget, int maxBudget, Map map)
        {
            if (!Core.AOS)
            {
                if (item is BaseJewel)
                {
                    var j = (BaseJewel)item;
                    j.SkillBonuses.Skill_1_Name = Utility.RandomSkill();
                    j.SkillBonuses.Skill_1_Value = 1;
                }
                if (item is BaseWeapon)
                {
                    var arma = (BaseWeapon)item;
                    if (Utility.RandomDouble() < 0.2)
                        arma.DamageLevel = (WeaponDamageLevel)Utility.Random(4);
                    if (Utility.RandomDouble() < 0.8)
                        arma.DurabilityLevel = (WeaponDurabilityLevel)Utility.Random(4);
                    if (Utility.RandomDouble() < 0.3)
                        arma.AccuracyLevel = (WeaponAccuracyLevel)Utility.Random(4);
                    if (Utility.RandomDouble() < 0.3)
                        arma.Slayer = BaseRunicTool.GetRandomSlayer();
                    if (Utility.RandomDouble() < 0.1)
                        arma.Slayer2 = BaseRunicTool.GetRandomSlayer();
                ...
                {
                    var armadura = (BaseArmor)item;
                    if (Utility.RandomDouble() < 0.2)
                        armadura.ProtectionLevel = (ArmorProtectionLevel)Utility.Random(4);
                    if (Utility.RandomDouble() < 0.8)
                        armadura.Durability = (ArmorDurabilityLevel)Utility.Random(4);
                    if (Utility.RandomDouble() < 0.4)
                        armadura.Resource = CraftResource.Bronze;
                    else if (Utility.RandomDouble() < 0.2)
                        armadura.Resource = CraftResource.Dourado;
                    else if (Utility.RandomDouble() < 0.2)
                        armadura.Resource = CraftResource.Niobio;
                    if (Utility.RandomDouble() < 0.2)
                        armadura.Quality = ItemQuality.Exceptional;
                }
                else if (item is BaseInstrument)
                {
                    var instr = (BaseInstrument)item;
                    if (Utility.RandomDouble() < 0.4)
                        instr.Slayer = BaseRunicTool.GetRandomSlayer();
                    if (Utility.RandomDouble() < 0.1)
                        instr.Slayer2 = BaseRunicTool.GetRandomSlayer();
                }
                return true;
            }
