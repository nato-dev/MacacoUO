...
namespace Server.Engines.CityLoyalty
{
    ...
    {
        public static readonly bool Enabled = false;// Config.Get("CityLoyalty.Enabled", true);
        ...
        public Mobile Governor
        {
            ...
            set
            {
                if (_Governor != null && _Governor != value && _Governor.NetState != null)
                    _Governor.SendLocalizedMessage("Voce foi removido do cargo de Governador"); // King Blackthorn thanks you for your service. You have been removed from the Office of the Governor.

                if (value == _GovernorElect)
                    _GovernorElect = null;

                if (value != null && value != _Governor)
                    HeraldMessage("Extra ! Extra ! " + value.Name + " foi eleito governador !"); // Hear Ye! Hear Ye! ~1_NAME~ hath accepted the Office of Governor! King Blackthorn congratulates Governor ~1_NAME~! 
        ...
        {
            if (from.AccessLevel > AccessLevel.VIP && staffIsCitizen)
                return true;
        ...
        public virtual void AwardHate(Mobile from, double hate)
        {
            ...
            {
                from.SendMessage("Seus feitos com Rhodes geram raiva nas pessoas"); // Your deeds in the city of ~1_name~ are worthy of censure.
            }
        ...
        public virtual void AwardLove(Mobile from, double love, bool message = true)
        {
            ...
            {
                from.SendMessage("Seus feitos em Rhodes geram alegrias"); // Your deeds in the city of ~1_name~ are worthy of praise.
            }
        ...
        public void TryUtilizeTradeDeal(Mobile m)
        {
            ...
            if (entry != null)
            {
                if (ActiveTradeDeal == TradeDeal.None)
                {
                    HeraldMessage("A cidade nao tem acordo comercial, talvez o governador deva fazer um ?"); // The City doth nay currently have a Trade Deal in place, perhaps ye should petition the Governor to make such a deal...
                }
                else if (entry.UtilizingTradeDeal)
                {
                    HeraldMessage("Voce ja fez uso do acordo comercial hoje senhor"); // Thou hath already claimed the benefit of the Trade Deal today!
                }
                else if (entry.LoyaltyRating < LoyaltyRating.Respected)
                {
                    HeraldMessage("Voce precisa ser pelo menos respeitado para usar do acordo comercial"); // Begging thy pardon, but thou must be at least Respected within the City to claim the benefits of a Trade Deal!
                }
                else
                {
                    ...
                    m.SendMessage("Voce ganhou o beneficio do acordo comercial"); // You gain the benefit of your City's Trade Deal!
                }
            }
        }
        ...
        public static void OnLogin(LoginEventArgs e)
        {
            ...
            if (sys != null)
            {
                ...
                {
                    message = "Sua lealdade a cidade continua mantida"; // The moons of Trammel and Felucca align to preserve your virtue status and city loyalty.
                }
                else
                {
                    message = "Sua lealdade a cidade esta em jogo"; // The moons of Trammel and Felucca fail to preserve your virtue status and city loyalty.
                }
        ...
        public static bool ApplyCityTitle(PlayerMobile pm, ObjectPropertyList list, string prefix, int loc)
        {
            ...
            {
                if (pm.Female)
                    list.Add(1151487, "{0} \t{1} a \t#{2}", prefix, pm.Name, loc);
                else
                    list.Add(1151487, "{0} \t{1} o \t#{2}", prefix, pm.Name, loc); // ~1NT_PREFIX~~2NT_NAME~~3NT_SUFFIX~
                return true;
            }
