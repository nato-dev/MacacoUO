...
namespace Server.Engines.Craft
{
    ...
    public class CraftItem
    {
        ...
        public CraftSkillCol m_arCraftSkill;
        ...
        public CraftItem(Type type, TextDefinition groupName, TextDefinition name)
        {
            ...
            var trans = Lang.Trans(m_GroupNameNumber);

            if (trans != null)
            {
                m_GroupNameNumber = 0;
                m_GroupNameString = trans;
            }
        ...
        public bool NeedForno { get { return m_NeeedForno; } set { m_NeeedForno = value; } }
        ...
        private static readonly int[] m_Fornos = new[]
        {
            0x9CE9, 0x9CE9
        };
        ...
        private static readonly Type[] m_MarkableTable = new[]
        {
            ...
            typeof(BarrelLid), typeof(Clippers), typeof(Scissors), typeof(BaseJewel),
            ...
            bool isFailure)
        {
            ...
            {
                message = "Voce precisa estar perto de um fogo"; // You must be near a fire source to cook.
                return false;
            }

            if (m_NeedOven && !Find(from, m_Ovens))
            {
                message = "Voce precisa de um forno para cozinhar isto"; // You must be near an oven to bake that.
                return false;
            }

            if (m_NeedMaker && !Find(from, m_Makers))
            {
                message = "Voce precisa cozinhar isso a vapor"; // You must be near a steam powered beverage maker to do that.
                return false;
            }

            if (m_NeedMill && !Find(from, m_Mills))
            {
                message = "Voce precisa de um moinho para cozinhar isto"; // You must be near a flour mill to do that.
                return false;
            }

            if (m_NeeedForno && !Find(from, m_Fornos))
            {
                message = "Voce precisa de um forno mecanico para isto"; // You must be near a flour mill to do that.
                return false;
            }
            ...
            {
                message = "Voce precisa de um caldeirao para isto.";
                return false;
            }
            if (m_NeedDistillery && !Find(from, m_Distillerys))
            {
                message = "Voce precisa de uma destilaria para isto.";
                return false;
            }
            if (m_NeedSteamPoweredBeverageMaker && !Find(from, m_SteamPoweredBeverageMakers))
            {
                message = "Voce precisa de uma maquina a vapor para isto.";
                return false;
            }
            #endregion
            ...
            Mobile from, Type typeRes, CraftSystem craftSystem, bool gainSkills, ref bool allRequiredSkills)
        {
            ...
            for (int i = 0; i < m_arCraftSkill.Count; i++)
            {
                ...
                double skillCrafting = from.Skills[craftSkill.SkillToMake].Value;

                if (skillCrafting < minSkill)
                {
                ...
                if (gainSkills) // This is a passive check. Success chance is entirely dependant on the main skill
                {
                    ...
                    else if (skillCrafting >= skillNovosResuorces)
                    {
                    ...
                    {
                        if (res > CraftResource.Cobre && res < CraftResource.Adamantium)
                        {
                            if (res < CraftResource.Cobre)
                            {
                                gainSkills = false;
                                from.SendMessage("Voce precisa trabalhar com pelo menos Cobre para subir sua skill");
                            }
                        }
                        if (res > CraftResource.Cedro && res < CraftResource.Gelo)
                        {
                            if (res < CraftResource.Cedro)
                            {
                                gainSkills = false;
                                from.SendMessage("Voce precisa trabalhar com pelo menos Cedro para subir sua skill");
                            }
                        }
                    }
                    else if (valSkill > 70)
                    ...
                    */
                    if (gainSkills)
                    {
                        ...
                        else
                        {
                            if (n > 0)
                            {
                                from.CheckSkillMult(craftSkill.SkillToMake, minSkill, maxSkill, n + 1);
                                from.SendMessage("Voce consegue aprender mais por usar um material mais dificil");
                            }
                            else
                            {
                                from.CheckSkillMult(craftSkill.SkillToMake, minSkill, maxSkill);
                            }
                        }
            ...
            if (typeRes != null)
            {
                ...
                else if (res == CraftResource.Lazurita)
                {
                    chance -= 0.2;
                }
            ...
            Shard.Debug("Chance Craft: " + chance);
            return chance;
        }
                    ...
                    if (allRequiredSkills && chance >= 0.0)
                    {
                        if (Recipe == null || !(from is PlayerMobile) || ((PlayerMobile)from).HasRecipe(Recipe))
                        {
                            if (!RequiresBasketWeaving || (from is PlayerMobile && ((PlayerMobile)from).BasketWeaving))
                            {
                                ...
                                {
                                    from.EndAction(typeof(CraftSystem));
                                    from.SendGump(new CraftGump(from, craftSystem, tool, "Voce precisa aprender o manual de mecanica goblin para isto")); // You haven't read the Mechanical Life Manual. Talking to Sutek might help!
                                }
                            }
                            else
                            {
                                from.EndAction(typeof(CraftSystem));
                                from.SendGump(new CraftGump(from, craftSystem, tool, "Voce precisa aprender a arte de fazer cestos para isto")); // You haven't learned basket weaving. Perhaps studying a book would help!
                            }
                        }
                        else
                        {
                            from.EndAction(typeof(CraftSystem));
                            from.SendGump(new CraftGump(from, craftSystem, tool, "Voce precisa aprender a fazer isto obtendo um pergaminho de receita")); // You must learn that recipe from a scroll.
                        }
                    }
                    else
                    {
                        from.EndAction(typeof(CraftSystem));
                        from.SendGump(new CraftGump(from, craftSystem, tool, "Voce nao tem habilidades suficiente para fazer isto"));
        ...
        private object RequiredExpansionMessage(Expansion expansion)
        {
            ...
            {
                case Expansion.SE:
                    return "Falar esse numero pro admin: 14515"; // The "Samurai Empire" expansion is required to attempt this item.
                case Expansion.ML:
                    return "Falar esse numero pro admin: 15151"; ; // The "Mondain's Legacy" expansion is required to attempt this item.
                case Expansion.SA:
                    return "Falar esse numero pro admin: 34564"; ; // You must have the Stygian Abyss expansion pack to use this feature.
                case Expansion.HS:
                    return "Falar esse numero pro admin: 234234"; ; // You must have the High Seas booster pack to use this feature
                case Expansion.TOL:
                    return "Falar esse numero pro admin: 657567"; ; // You must have the Time of Legends expansion to use this feature.
                default:
                    return "Falar esse numero pro admin: 657567";
            }
        }
            ...
            CustomCraft customCraft)
        {
            ...
            if (CheckSkills(from, typeRes, craftSystem, ref ignored, ref allRequiredSkills))
            {
                ...
                if (item != null)
                {
                    ...
                    {
                        EventSink.InvokeCraftSuccess(new CraftSuccessEventArgs(from, item, (Item)tool, this));
                    }
            ...
            protected override void OnTick()
            {
                ...
                else
                {
                    ...
                    if (badCraft > 0)
                    {
                        ...
                        {
                            DefBlacksmithy.Translate(m_From, badCraft);
                        }
                    ...
                    if (quality == 2 && m_From.Skills[skill].Base >= 100.0)
                    {
