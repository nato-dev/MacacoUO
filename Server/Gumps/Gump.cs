...
namespace Server.Gumps
{
    public class Gump
    {
        ...
        public void AddHtmlLocalized(int x, int y, int width, int height, string number, bool background, bool scrollbar)
        {
            Add(new GumpHtml(x, y, width, height, number, background, scrollbar));
        }

        public void AddHtml(int x, int y, int width, int height, string number, int color, bool background, bool scrollbar)
        {
            if (color == 0x0)
            {
                color = 1;
            }
            Add(new GumpHtml(x, y, width, height, Cor(number, color), background, scrollbar));
        }

        public static int ToRgb(Color value)
        {
        ...
        public static string Cor(string text, int color)
        {
            ...
            var c = HueToColor((short)color);
            return String.Format("<basefont color=#{0:X6}>{1}</basefont>", ToRgb(c), text);
            ...
            bool scrollbar)
        {
            ...
            if (str != null)
            {
                Add(new GumpHtml(x, y, width, height, Cor(str, color), background, scrollbar));
            }
            else
