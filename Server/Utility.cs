...
namespace Server
{
    public static class Utility
    {
        ...
        public static bool InUpdateRange(Mobile m, Point3D p, IPoint2D o)
        {
            if (o is Item i)
                return Utility.InRange(p, i.GetWorldLocation(), i.GetUpdateRange(m));

            return Utility.InRange(p, o, m.NetState?.UpdateRange ?? Core.GlobalUpdateRange);
        }

        public static bool InUpdateRange(Mobile m, IPoint2D p)
        {
            if (p is Item item)
                return Utility.InRange(m.Location, item.GetWorldLocation(), item.GetUpdateRange(m));

            return Utility.InRange(m.Location, p, m.NetState?.UpdateRange ?? Core.GlobalUpdateRange);
        }
...
#pragma warning restore 618
        ...
        public static void WriteConsoleColor(ConsoleColor color, string text, params object[] args)
        {
