...
namespace Server
{
    ...
    public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable, IDamageable
    {
        ...
        public List<Type> Last5Spells = new List<Type>();
        public List<DateTime> SpellTimes = new List<DateTime>();

        public int GetRepeatedTypes(Type spellType, TimeSpan interval)
        {
            var repeated = 0;
            var timeLimit = DateTime.UtcNow - interval;
            for (var i = 0; i < Last5Spells.Count; i++)
            {
                var lastSpell = Last5Spells[i];
                var lastTime = SpellTimes[i];
                if (lastSpell == spellType && lastTime > timeLimit)
                {
                    repeated++;
                }
            }
            Shard.Debug("Repeated spells: " + repeated);
            return repeated;
        }

        public DateTime LastParalized = DateTime.MinValue;

        public void AddLastSpell(Type t)
        {
            Shard.Debug("Added to spell history " + t.Name, this);
            Last5Spells.Add(t);
            SpellTimes.Add(DateTime.UtcNow);
            if (Last5Spells.Count > 5)
            {
                Last5Spells.RemoveAt(0);
                SpellTimes.RemoveAt(0);
            }
        }
        ...
        public bool m_Female, m_Warmode, m_Hidden, m_Blessed, m_Flying;
        ...
        public virtual int Tilematrix { get { return 0; } set { } }
		...
		*/
        ...
        private static int m_ActionMessageDelay = 65;
        ...
        public virtual void SendSkillMessage()
        {
            ...
            SendMessage("Aguarde para usar uma skill novamente"); // You must wait a few moments to use another skill.
        }
        ...
        public virtual void SendActionMessage()
        {
            ...
            SendMessage("Voce precisa aguardar para fazer outra acao"); // You must wait to perform another action.
        }
        ...
        private class ManaTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                {
                    m_Owner.Mana += m_Owner.Meditating ? 4 : 2;
                }
        ...
        private class CombatTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                if (Shard.TROCA_ARMA_RAPIDA && m_Mobile.Weapon != null)
                {
                    ...
                    {
                        //Shard.Debug("Ktei delay da arma "+aux+" antigo "+ m_Mobile.m_NextCombatTime);
                        m_Mobile.m_NextCombatTime = aux;
                    }
                }
        ...
        public virtual void OnBeforeAttack(Mobile target) { }
        ...
        public virtual void AggressiveAction(Mobile aggressor, bool criminal)
        {
            ...
            Shard.Debug("Agressor: " + aggressor.Name, this);
        ...
        public virtual bool Move(Direction d, bool ignoreDelay = false)
        {
            if (m_Deleted)
            {
                return false;
            ...
            if ((m_Direction & Direction.Mask) == (d & Direction.Mask))
            {
                ...
                int newZ;
                bool moveCheck = CheckMovement(d, out newZ);
                if (moveCheck)
                {
                    ...
                    if (map != null)
                    {
                        ...
                        var mudandoSector = oldSector != newSector;
            ...
            if (this.Direction == d && m_Spell != null)
            {
                m_Spell.DoStep(this);
            }
            SetDirection(d);
        ...
        public virtual bool CheckShove(Mobile shoved)
        {
            ...
            else if (shoved.m_Hidden || shoved.IsStaff() || !this.CanSee(shoved))
            {
                return true;
            }
        ...
        public virtual void CriminalAction(bool message)
        {
            ...
            this.PrivateOverheadMessage("[ Criminoso ]");
        ...
        public virtual bool CanUseStuckMenu()
        {
            ...
            else
            {
                ...
                {
                    if ((DateTime.UtcNow - m_StuckMenuUses[i]) > TimeSpan.FromHours(1))
                    {
                        return true;
                    }
                }
        ...
        public virtual void Lift(Item item, int amount, out bool rejected, out LRReason reject)
        {
            ...
            if (from.IsStaff() || Core.TickCount - from.NextActionTime >= 0)
            {
                if (from.CheckAlive())
                {
                    ...
                    //Alterado para não parar de meditar ao mover um item

                    Shard.Debug("Drag item liftando", this);
                    ...
                    else
                    {
                        ...
                        {
                            Shard.Debug("Liftando stealing " + item.Name, from);
                            reject = LRReason.TryToSteal;
                        }
                        else if (!from.OnDragLift(item) || !item.OnDragLift(from))
                        {
                            Shard.Debug("Drag Lift negado", this);
                            reject = LRReason.Inspecific;
                        }
                        ...
                        else
                        {
                            ...
                            Shard.Debug("Liftou item " + item.Name, this);
                            reject = LRReason.Inspecific;
                            rejected = false;
                        }
                    }
            ...
            {
                Shard.Debug("Lift Negado: " + reject, this);
                ...
                for (int i = 0; i < hears.Count; ++i)
                {
                    ...
                    if (mutatedArgs == null || !CheckHearsMutatedSpeech(heard, mutateContext))
                    {
                        ...
                        if (ns != null)
                        {
                            if (regp == null)
                            {
                                //var p = new AsciiMessage(m_Serial, Body, type, hue, 3, Name, text);
                                //regp = Packet.Acquire(p);
                ...
                case VisibleDamageType.Related:
                    {
                        ...
                        if (amount > 0 && (ourState != null || theirState != null))
                        {
                            ...
                            if (theirState != null && theirState != ourState)
                            {
                                ...
                                Console.WriteLine("DMG PACKET " + newPacket);
        ...
        public virtual void RevealingAction(bool msg = true)
        {
            ...
            //Alterado para não interromper medit com ações que revelam o player

            if (wasHidden && !Hidden && msg)
                this.OverheadMessage("* Revelado *");
        ...
        public virtual void OnSaid(SpeechEventArgs e)
        {
            if (m_Squelched)
            {
                ...
                {
                    SendMessage("Voce nao pode falar nada, esta mutado."); //Cliloc ITSELF changed during ML.
                }
        ...
        /// </summary>
        [CommandProperty(AccessLevel.GameMaster)]
        public virtual int HitsMax { get { return 50 + ((Str + Dex + Int) / 225) * 50; } }
        ...
        public int Mana
        {
            ...
            set
            {
                ...
                else if (value >= ManaMax)
                {
                    ...
                    {
                        Meditating = false;
                        SendMessage("Voce esta em paz"); // You are at peace.
                        PlaySound(0x0F5);
                    }
                }
        ...
        {
            PublicOverheadMessage(MessageType.Emote, 0xB7, true, "O veneno nao faz efeito"); // * The poison seems to have no effect. *
        }
        ...
        public virtual void OnPoisoned(Mobile from, Poison poison, Poison oldPoison)
        {
            if (poison != null)
            {
                if ((poison != oldPoison && oldPoison == null) || (oldPoison != null && oldPoison.Level < poison.Level))
                {
                    PrivateOverheadMessage("* Voce foi envenenado *");
                }
                else
                {
                    if (poison == Poison.Deadly)
                        PrivateOverheadMessage("* Voce sente o veneno mortal correr em suas veias *");
                    else if (poison == Poison.Greater)
                        PrivateOverheadMessage("* Voce se sente muito doente *");
                    else if (poison == Poison.Regular)
                        PrivateOverheadMessage("* Voce se sente doente *");
                    else if (poison == Poison.Lesser)
                        PrivateOverheadMessage("* Voce se sente doente *");
                    else if (poison == Poison.Lethal)
                        PrivateOverheadMessage("* Voce sente seu coração parar e sua vida se esvair *");
                    else
                        PrivateOverheadMessage("* Voce esta passando mal *");
                }

                // OthersOverheadMessage("* Envenenado *");
                /*
                if (poison == Poison.Deadly)
                    OthersOverheadMessage("* Sentindo o veneno mortal em suas veias *");
                else if(poison == Poison.Greater)
                    OthersOverheadMessage("* Se sentindo muito doente *");
                else if (poison == Poison.Regular)
                    OthersOverheadMessage("* Se sentindo doente *");
                else if (poison == Poison.Lesser)
                    OthersOverheadMessage("* Se sentindo levemente doente *");
                else if (poison == Poison.Lethal)
                    OthersOverheadMessage("* Sentindo o coracao parar *");
                else
                    OthersOverheadMessage("* Passando mal *");
                */
        ...
        public Point3D LastDungeonEntrance = Point3D.Zero;
        ...
        #region Simple Cooldowns (no save)
        ...
        public bool CheckAlive(bool message)
        {
            if (!Alive)
            {
                if (message)
                {
                    LocalOverheadMessage(MessageType.Regular, 0xB7, 1019048); // I am dead and cannot do that.
                }
        ...
        public static bool IsLightLandSwamp(int itemID)
        {
        ...
        public static bool IsDeepLandSwamp(int itemID)
        {
        ...
        public static bool IsDirtLand(int itemID)
        {
        ...
        public static bool IsSandLand(int itemID)
        {
        ...
        public static bool IsSnowLand(int itemID)
        {
        ...
        public static bool IsTileLightWoodenFloor(int itemID)
        {
        ...
        public static bool IsTileHardStoneFloor(int itemID)
        {
        ...
        public static bool IsLightStaticSwamp(int itemID)
        {
        ...
        public static bool IsDeepStaticSwamp(int itemID)
        {
        ...
        public static bool IsStaticLightWoodenFloor(int itemID)
        {
        ...
        public static bool IsStaticDarkWoodenFloor(int itemID)
        {
        ...
        public static bool IsStaticBambooFloorAndSteps(int itemID)
        {
        ...
        public static bool IsStaticHardStoneFloor(int itemID)
        {
        ...
        public static bool IsStaticBushes(int itemID)
        {
        ...
        public static bool IsShallowWaterSurface(int itemID)
        {
        ...
        public static bool IsDeepWaterSurface(int itemID)
        {
        ...
        public void PublicOverheadMessage(MessageType type, int hue, int number, string args, bool noLineOfSight)
        {
            ...
            if (str != null)
            {
                PublicOverheadMessage(type, hue, true, str, noLineOfSight);
                return;
            }
        ...
        public void PrivateOverheadMessage(MessageType type, int hue, int number, string args, NetState state)
        {
            ...
            if (str != null)
            {
                PrivateOverheadMessage(type, hue, true, str, state);
                return;
            }
        ...
        {
            var trans = Lang.Trans(number);
            if (trans != null)
            {
                PrivateOverheadMessage(trans, hue);
                return;
            }
        ...
        public void OthersOverheadMessage(string msg)
        {
            NonlocalOverheadMessage(MessageType.Regular, 0, true, msg);
        }

        public void PrivateOverheadMessage(string msg, int hue = 0)
        {
        ...
        public void NonlocalOverheadMessage(MessageType type, int hue, int number, string args)
        {
            ...
            if (str != null)
            {
                NonlocalOverheadMessage(type, hue, true, str);
                return;
            }
        ...
        public void SendLocalizedMessage(string number)
        {
            SendMessage(number);
        }
        ...
        public static bool DisableHiddenSelfClick { get { return false;/* return m_DisableHiddenSelfClick;*/  } set { m_DisableHiddenSelfClick = value; } }
        ...
        public virtual void OnSingleClick(Mobile from)
        {
            ...
            {
                Shard.Debug("Single click negado", this);
                return;
            }

            //Shard.Debug("Construindo string pra aparecer em cima do mobile");
            ...
            else
            {
                ...
                else if (noto == Notoriety.Criminal && this.Player)
                {
        ...
        public bool CheckSkillMult(SkillName skill, double minSkill, double maxSkill, double mult = 1)
        {
            ...
            {
                return m_SkillCheckLocationHandler(this, skill, minSkill, maxSkill, mult);
            }
        }
        ...
        public bool CheckSkill(SkillName skill, double chance, double mult = 1)
        {
            ...
            {
                return m_SkillCheckDirectLocationHandler(this, skill, chance, mult);
            }
        }
        ...
        public bool CheckTargetSkillMinMax(SkillName skill, object target, double minSkill, double maxSkill, double mult = 1)
        {
            ...
            {
                return m_SkillCheckTargetHandler(this, skill, target, minSkill, maxSkill, mult);
            }
        }
        ...
        public bool CheckTargetSkill(SkillName skill, object target, double chance, double mult = 1)
        {
            ...
            {
                return m_SkillCheckDirectTargetHandler(this, skill, target, chance, mult);
            }
        }

        public virtual void DisruptiveAction(bool cancelMedit = true)
        {
            if (cancelMedit && Meditating)
            {
                Meditating = false;
                SendMessage("Você parou de meditar"); // You stop meditating.
            }
        }
